#!/bin/bash
# build.sh — perl-testharness (Layer 0: Perl leaf module, no PVE deps)
#
# Test::Harness — Run Perl test scripts with statistics.
# CPAN-sourced: https://metacpan.org/pod/Test::Harness
# Note: Tests are skipped (doCheck = false in Nix) as this is a build dependency.
#
...
CPAN_URL="https://www.cpan.org/authors/id/L/LE/LEONT/Test-Harness-3.52.tar.gz"
...
mv "/tmp/src/Test-Harness-3.52" "$WORKDIR"
cd "$WORKDIR"

# ------------------------------------------------------------------
# 2. Build the Perl module (skip tests per Nix)
# ------------------------------------------------------------------
...
echo "Test::Harness — Perl module to run test scripts with statistics" > "$STAGE/meta/description"
